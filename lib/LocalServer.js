...
function init(extensionContext) {
	...
	if (userConfig.localMode) {
		...
		request.get(_getKillURL(), { qs: { token: userConfig.token } }, (err, res, bd) => {
			...
			var result = {};
			try { result = JSON.parse(bd); } catch (e) { log.error('[Error]: parse JSON failed!'); }
			if (result.success) {
...
function startLocalServer(silent) {
	...
	function onServerStdout(data) {
		data = String(data);
		if (!ENABLE_PIPE_SERVER_OUTPUT && data.indexOf('Server started!') < 0)
			return;
		data.split('\n').forEach(it => log.debug(`[LocalServer/stdout]: ${it}`));
	}
	function onServerStderr(data) {
		String(data).split('\n').forEach(it => log.error(`[LocalServer/stderr]: ${it}`));
	}
}
